...
/// the story's settings, reading its topics from their end, so what the scenario publishes after the start is
/// what the story sees. The container's log is the story's voice; the scenario reads it. Stopped when the
/// scenario ends.
@Component
@ScenarioScope
public class StoryContainer {
    ...
    /// the story reads from the end of its topics: a fresh group starts there
    private static final String FROM_THE_END = "--spring.kafka.consumer.auto-offset-reset=latest";
    ...
    public void start(String story, Map<String, String> settings) {
        var command = Stream.concat(
                        Stream.of("--story=" + story, FROM_THE_END),
    ...
    /// Stops the story the way a deployment does, SIGTERM first: the context closes and the consumer leaves its
    /// group, so the next scenario's story finds the group empty. Then the container goes.
    @PreDestroy
    public void stop() {
        if (container != null) {
