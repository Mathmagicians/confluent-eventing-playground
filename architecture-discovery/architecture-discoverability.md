...
5. **Documents.** The same test class carries a tagged writer, run by the Gradle task `generateArchitecture`. It
   ...
   around the domain. Inside each module, every type with a stereotype is a card with its role, laid out in a
   grid, the permitted types of a sealed type after it, implementing it. `HexagonDocumenter.Options` caps the
   types listed per role, unlimited by default, an ellipsis line with the total beyond the cap.
