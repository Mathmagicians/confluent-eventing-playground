...
import dk.mathmagicians.playground.confluent.eventing.adapter.kafka.Converter;
...
class Cluster {
    ...
    ConsumerRecord<String, byte[]> assertEnvelopeAt(String topic, Receipt receipt) {
        ...
        assertThat(header(record, Converter.ID)).isEqualTo(receipt.envelopeId());
        return record;
    }

    /// The envelope in the record's headers names the region.
    void assertRegion(ConsumerRecord<String, byte[]> record, String region) {
        assertThat(header(record, Converter.REGION)).isEqualTo(region);
    }
    ...
    private ConsumerRecord<String, byte[]> read(String topic, int partition, long offset) {
